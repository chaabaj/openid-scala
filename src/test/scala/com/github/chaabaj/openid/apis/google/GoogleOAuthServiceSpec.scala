...
class GoogleOAuthServiceSpec extends Specification with Mockito {
  ...
  "should retrieve token" >> {
    ...
    val token = Await.result(service.issueOAuthToken("test", "http://test.com"), 10 seconds)
    val expectedToken = response.convertTo[OAuthTokenIssuing]

    token.accessToken must equalTo(expectedToken.accessToken)
    token.tokenType must equalTo(expectedToken.tokenType)
    token.expireIn must equalTo(expectedToken.expireIn)
    token.refreshToken must equalTo(expectedToken.refreshToken)
  }
  ...
  "should fails to retreive a token with an OAuthException" >> {
    ...
    Await.result(service.issueOAuthToken("test", "http://test.com"), 10 seconds) must throwA[OAuthException]
  }
  ...
  "should fails with a RuntimeException" >> {
    ...
    Await.result(service.issueOAuthToken("test", "http://test.com"), 10 seconds) must throwA[RuntimeException]
  }
}
